...
class Bridge(QObject):
    ...
    def get_image(self, image_attr: str) -> str:
        """Return a base64 encoded string of a graphic file from the package. Read the image attribute directly."""
        try:
            _group_id, _instance_id = image_attr[1:-1].split(",")
            group_id = int(_group_id, 16)
            instance_id = int(_instance_id, 16)
        except ValueError:
            print(f"Invalid image ID: {image_attr}")
            return ""
